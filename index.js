    ...
    client.on('data', function(data) {
      ...
      for (var i = 0; i < commands.length; i++) {
        if (commands[i].seq === seq) {
          if (!commands[i].processer || !commands[i].processer(commands[
              i], data)) {
            commands.splice(i, 1);
          }
          break;
...
function node_on_off(mac, on) {
  return new Promise(function(resolve, reject) {
    ...
    var cmd = create_command(COMMAND_ONOFF, body)
      .setprocesser(successResponseProcesser.bind({
        resolve: resolve,
        reject: reject
      }));
...
var exports = module.exports = {
  ...
  isBrightnessSupported: function(type) {
    return getNodeType(type) === 2 || getNodeType(type) === 4 || (
      getNodeType(type) != 16 && getNodeType(type) != 1);
  },
  isTemperatureSupported: function(type) {
    return getNodeType(type) === 2 || getNodeType(type) === 10;
  },
